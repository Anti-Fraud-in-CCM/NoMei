...
def get_data():
    sql = "select id, repo_id, label, isFirst_forest, isSecond_forest, isThird, isForth_forest" \
...
def update_isBad(id, prediction):
    sql = "update message set isBad_forest = %d where id = %d" % (prediction, id)
    # print(sql)
    try:
        cursor.execute(sql)
        db.commit()
